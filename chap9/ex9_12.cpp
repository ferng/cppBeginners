...
Set Set::operator &(Set ob2)
{
	...
	for (i=0; i<len; i++)
		if (ob2.isMember(members[i]))
			newSet.members[newSet.len++] = members[i];
...
int main()
{
	...
	s3=s1&s2;
	cout << endl;
	cout << "the junction of s1 and s2 is: ";
	s3.showSet();
	cout << endl;

	s3=s1|s2;
	cout << "the symetric difference of s1 and s2 is: ";
	s3.showSet();
	cout << endl;
}
